...
class TahunAjaran extends \yii\db\ActiveRecord
{
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if($insert){
            ...
            $command->update($this::tableName(),['status' => 0],['<>','id',$this->id])->execute();
            $command->update($this::tableName(),['status' => 1],['id' =>$this->id])->execute();
        }
    }
...
//        $command->update($this::tableName(),['status' => 0]);
    ...
    private function addSaldoRekening(){
        $command = Yii::$app->db->createCommand();
        $tahunAjaran = $this->id;
        $activeTahunajaran = self::getActive();
        $idTahunAjaran =  $activeTahunajaran? $activeTahunajaran->id:null;
        if($idTahunAjaran){
            $command->setSql("INSERT INTO keu_saldorekening(detilrekening,tahunajaran,nominal)
              SELECT detilrekening,{$tahunAjaran},nominal FROM keu_saldorekening where tahunajaran =  {$idTahunAjaran} 
            ");
        }else{
            $tahunAjaran = $this->id;
            $command->setSql("INSERT INTO keu_saldorekening(detilrekening,tahunajaran,nominal)
    ...
    public function rules()
    {
        return [
            ...
            [['status'],function($attribute, $params, $validator){
                ...
                if($active){
                    if(($active->id != $this->id) && $this->$attribute == 1){
                        $this->addError($attribute, 'Status tidak dapat diaktifkan karena ada tahun ajaran lain yang masih aktif.');
                    }
                }
